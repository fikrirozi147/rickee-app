...
export default function Scanner() {
  ...
  return (
    // 🍞 UPDATED: Using the "Golden Crust" Gradient
    <LinearGradient 
      colors={['#2E2115', '#6D4C41', '#D4A373']} // Dark Espresso -> Cocoa -> Golden Wheat
      start={{ x: 0, y: 0 }}
      end={{ x: 1, y: 1 }}
    ...
    >
      {mode === 'camera' ? (
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.manualContainer}>
          ...
          <BlurView intensity={20} tint="light" style={styles.glassInputContainer}>
            <TextInput 
              style={styles.input} 
      ...
      {/* GLASS DOCK */}
      <BlurView intensity={40} tint="dark" style={styles.glassDock}>
        <TouchableOpacity style={[styles.dockItem, mode === 'camera' && styles.activeDockItem]} onPress={() => setMode('camera')}>
          <Ionicons name="camera" size={24} color={mode === 'camera' ? "#fff" : "#DDD"} />
        ...
        <TouchableOpacity style={[styles.dockItem, mode === 'manual' && styles.activeDockItem]} onPress={() => setMode('manual')}>
          <Ionicons name="keypad" size={24} color={mode === 'manual' ? "#fff" : "#DDD"} />
...
const styles = StyleSheet.create({
  container: { flex: 1 }, // No background color needed, LinearGradient handles it
  ...
  manualTitle: { fontSize: 32, fontWeight: 'bold', color: 'white', marginBottom: 20, textAlign: 'center', textShadowColor: 'rgba(0,0,0,0.3)', textShadowRadius: 5 },
  glassInputContainer: { borderRadius: 15, padding: 5, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.3)', marginBottom: 20 },
  ...
  dockItem: { alignItems: 'center', opacity: 0.7 },
